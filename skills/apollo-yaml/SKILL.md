...
- **Write access:** `bharvey88` can push to most ApolloAutomation repos, so PRs usually branch directly on the Apollo repo; **H-2** has no write access (fork `bharvey88/H-2`, clone at `C:\tmp\H-2` with `upstream` = Apollo), **H-3** has push but not admin (can't enable Pages), and the org's **esphome fork (`ApolloAutomation/esphome`) is pull-only** for Brandon - he CAN create new org repos though (that's how `ApolloAutomation/esphome-components` came to exist, 2026-07-21). New org repos get a ruleset requiring changes to `main` via PR (initial push exempt). Local clones/worktrees are inconsistent about which remote is which (`origin` is sometimes the fork) - run `git remote -v` and confirm you're pushing to the remote the PR head actually lives on, or you'll create a stray fork branch instead of updating the PR.
...
- **Ornaments (H-1/H-2/H-3):** each has two variants, `H-x.yaml` (smart, wifi + HA, 90 s deep-sleep `run_duration`) and `H-xD.yaml` (the basic standalone firmware they ship with: no wifi, no OTA; published to Pages `firmware-d/`). Only the smart variant carries `improv_serial` (H-1 had it, H-2#31 / H-3#5 add it). H-3 is unreleased as of 2026-08-23 (no Pages, no release, no wiki page) but Brandon wants it worked on; it is not in the TEMP_PRO-1 "do not touch" bucket.
- **Which branch?** A firmware change → PR to **`beta`**, then STOP (a maintainer merges, never self-merge). This holds even though the org ruleset only locks `main` and a beta merge would succeed: on 2026-08-23 I merged H-1 #32/#31 into beta because "if the build succeeds we should just merge this" read as a go, and Brandon's reply was "you should not merge stuff directly into beta, that would be bad". "We should merge" means he merges. Report green CI and stop. A **workflow-only** change (`.github/workflows/*`, no firmware published) → target **`main`**, because `pull_request_target` / default-branch workflows only take effect on `main`.
...
- Date is **today's actual date** - re-check it, don't reuse the date already in the string. Re-check again at PR-open time: a session that crosses midnight ships yesterday's date (H-2#31 needed a second commit to move 26.8.22.1 to 26.8.23.1).
