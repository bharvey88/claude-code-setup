...
- **Quote every `description:` value in front matter.** YAML reads an unquoted scalar containing a colon followed by a space as a mapping, which invalidates the *entire* front matter block and silently drops the page back to `site_description`. There is no build warning and nothing looks wrong in the source. This shipped broken twice on 2026-08-22 (`multisensor: mmWave...`, `answered: which sensors...`). Only reading the built `og:description` catches it, so verify in the build output, not the markdown.
- **Adding front matter to a page that another page includes whole breaks the includer.** `--8<-- "products/x/y.md"` with no offset pastes the file verbatim, front matter and all, so it renders as visible text on the including page. Add the offset in the same commit: 3-line front matter (`---`, `description:`, `---`) needs `:4:`, the repo's older 4-line form (with `title:`) needs `:5:`. Grep for `--8<-- "<path>"` before adding front matter to anything.
- **`docs/_snippets/` and `docs/products/new-product-template/` are excluded from the build** via `exclude_docs` in mkdocs.yml (added 2026-08-22). Both were publishing as live pages in the sitemap. A new file in either directory will not become a page, which is the point. `pymdownx.snippets` still resolves `_snippets` includes because it reads from disk through its own `base_path`, not MkDocs' file collection.
- **Don't bulk-edit Markdown with `sed -i`.** On this repo `sed -i` rewrites line endings (CRLF→LF) on *every* file it scans, even no-match files, producing dozens of spurious diffs. Use the Edit/Write tools instead. If you must run a batch command, `git add` only the files you actually changed and confirm the real diff with `git diff --ignore-all-space --stat` before committing. **The same trap applies to Python.** Reading a file with `decode()`, editing the string, and writing it back re-normalizes line endings across the whole file: a two-line addition to `air1/faq.md` produced a 280-line diff on 2026-08-22. Edit in bytes (`io.open(p,'rb')`, `bytes.replace`, `io.open(p,'wb')`) and never round-trip through str.
