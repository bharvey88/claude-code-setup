...
- **An orphaned `mkdocs serve` can hold port 8000 and serve a stale build for hours.** A serve from an earlier session survives that session ending, so a new one logs "Serving on http://127.0.0.1:8000/" while the old process still answers. Symptom: pages you just created 404 and pages you deleted still load, on a build that was clean. Check with `Get-CimInstance Win32_Process -Filter "Name like '%python%'" | Where-Object { $_.CommandLine -like '*mkdocs*' }`, kill every match, then start one. Kill leftovers at session end too.
